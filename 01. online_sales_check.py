...
cols = ['매출일자','판매처','거래구분','품번','총 금액','자사몰 주문번호','자사몰 상세번호','제휴몰 주문번호','제휴몰 상세번호','귀속월','주문_상세']
어드민 = 어드민[cols]
# -

어드민_pvt = pd.pivot_table(어드민, index='주문_상세', values='총 금액', aggfunc='sum').reset_index()
...
어드민_인터_pvt = pd.pivot_table(어드민_인터, index='주문_상세', values='총 금액', aggfunc='sum').reset_index()
...
# +
인터_mg.loc[인터_mg['자료_총판매금액'].notnull(), '차이'] = 인터_mg['총 금액'] - 인터_mg['자료_총판매금액']
인터_mg.loc[인터_mg['현금입금'].notnull(), '차이'] = 인터_mg['총 금액'] - 인터_mg['현금입금']
...
# +
# 현재 상태 카테고리화 하기
인터_mg.loc[(인터_mg['총 금액'] == 0) & (인터_mg['자료_총판매금액'].isnull()), 'T/N'] = 'True'
인터_mg.loc[인터_mg['차이'] == 0, 'T/N'] = 'True'

인터_mg.loc[(인터_mg['총 금액'] < 0) & (인터_mg['자료_총판매금액'].isnull()), 'T/N'] = '초기'
인터_mg.loc[(인터_mg['총 금액'] == 0) & (인터_mg['자료_총판매금액'] > 0), 'T/N'] = '초기'

인터_mg.loc[(인터_mg['총 금액'] > 0) & (인터_mg['자료_총판매금액'].isnull())& (인터_mg['현금입금'].isnull()), 'T/N'] = '미정산'
...
어드민_이베이_pvt = pd.pivot_table(어드민_이베이, index='주문_상세', values='총 금액', aggfunc='sum').reset_index()
...
이베_mg.loc[이베_mg['총_결제금액'].notnull(), '차이'] = 이베_mg['총 금액'] - 이베_mg['총_결제금액']
이베_mg.loc[이베_mg['현금입금'].notnull(), '차이'] = 이베_mg['총 금액'] - 이베_mg['현금입금']

# +
# 현재 상태 카테고리화 하기

이베_mg.loc[(이베_mg['총 금액'] == 0) & (이베_mg['총_결제금액'].isnull()), 'T/N'] = 'True'
이베_mg.loc[이베_mg['차이'] == 0, 'T/N'] = 'True'

이베_mg.loc[(이베_mg['총 금액'] < 0) & (이베_mg['총_결제금액'].isnull()), 'T/N'] = '초기'
이베_mg.loc[(이베_mg['총 금액'] < 0) & (이베_mg['총_결제금액'] == 0), 'T/N'] = '초기'
이베_mg.loc[(이베_mg['총 금액'] == 0) & (이베_mg['총_결제금액'] > 0), 'T/N'] = '초기'

이베_mg.loc[(이베_mg['총 금액'] > 0) & (이베_mg['총_결제금액'].isnull())& (이베_mg['현금입금'].isnull()), 'T/N'] = '미정산'
...
쿠팡_mg.loc[쿠팡_mg['매출금액(D=A-B)'].notnull(), '차이'] = 쿠팡_mg['총 금액'] - 쿠팡_mg['매출금액(D=A-B)']
쿠팡_mg.loc[쿠팡_mg['현금입금'].notnull(), '차이'] = 쿠팡_mg['총 금액'] - 쿠팡_mg['현금입금']

# +
# 현재 상태 카테고리화 하기
쿠팡_mg.loc[(쿠팡_mg['총 금액'] == 0) & (쿠팡_mg['매출금액(D=A-B)'].isnull()), 'T/N'] = 'True'
쿠팡_mg.loc[쿠팡_mg['차이'] == 0, 'T/N'] = 'True'

쿠팡_mg.loc[(쿠팡_mg['총 금액'] < 0) & (쿠팡_mg['매출금액(D=A-B)'].isnull()), 'T/N'] = '초기'
쿠팡_mg.loc[(쿠팡_mg['총 금액'] < 0) & (쿠팡_mg['매출금액(D=A-B)'] == 0), 'T/N'] = '초기'
쿠팡_mg.loc[(쿠팡_mg['총 금액'] == 0) & (쿠팡_mg['매출금액(D=A-B)'] > 0), 'T/N'] = '초기'

쿠팡_mg.loc[(쿠팡_mg['총 금액'] > 0) & (쿠팡_mg['매출금액(D=A-B)'].isnull())& (쿠팡_mg['현금입금'].isnull()), 'T/N'] = '미정산'
...
어드민_패플_pvt = pd.pivot_table(어드민_패플, index='주문_상세', values='총 금액', aggfunc='sum').reset_index()
...
패플_mg.loc[패플_mg['판매금액'].notnull(), '차이'] = 패플_mg['총 금액'] - 패플_mg['판매금액']
패플_mg.loc[패플_mg['현금입금'].notnull(), '차이'] = 패플_mg['총 금액'] - 패플_mg['현금입금']

# +
# 현재 상태 카테고리화 하기
패플_mg.loc[(패플_mg['총 금액'] == 0) & (패플_mg['판매금액'].isnull()), 'T/N'] = 'True'
패플_mg.loc[패플_mg['차이'] == 0, 'T/N'] = 'True'

패플_mg.loc[(패플_mg['총 금액'] < 0) & (패플_mg['판매금액'].isnull()), 'T/N'] = '초기'
패플_mg.loc[(패플_mg['총 금액'] < 0) & (패플_mg['판매금액'] == 0), 'T/N'] = '초기'
패플_mg.loc[(패플_mg['총 금액'] == 0) & (패플_mg['판매금액'] > 0), 'T/N'] = '초기'

패플_mg.loc[(패플_mg['총 금액'] > 0) & (패플_mg['판매금액'].isnull())& (패플_mg['현금입금'].isnull()), 'T/N'] = '미정산'
...
어드민_패플_pvt = pd.pivot_table(어드민_패플, index='제휴몰 주문번호', values='총 금액', aggfunc='sum').reset_index()
# -
...
어드민_하프_pvt = pd.pivot_table(어드민_하프, index='주문_상세', values='총 금액', aggfunc='sum').reset_index()
...
# 하프_mg['차이'] = 하프_mg['총 금액'] - 하프_mg['실출고금액']
하프_mg.loc[하프_mg['실출고금액'].notnull(), '차이'] = 하프_mg['총 금액'] - 하프_mg['실출고금액']
하프_mg.loc[하프_mg['현금입금'].notnull(), '차이'] = 하프_mg['총 금액'] - 하프_mg['현금입금']

# +
# 현재 상태 카테고리화 하기
하프_mg.loc[(하프_mg['총 금액'] == 0) & (하프_mg['실출고금액'].isnull()), 'T/N'] = 'True'
하프_mg.loc[하프_mg['차이'] == 0, 'T/N'] = 'True'

하프_mg.loc[(하프_mg['총 금액'] < 0) & (하프_mg['실출고금액'].isnull()), 'T/N'] = '초기'
하프_mg.loc[(하프_mg['총 금액'] < 0) & (하프_mg['실출고금액'] == 0), 'T/N'] = '초기'
하프_mg.loc[(하프_mg['총 금액'] == 0) & (하프_mg['실출고금액'] > 0), 'T/N'] = '초기'

하프_mg.loc[(하프_mg['총 금액'] > 0) & (하프_mg['실출고금액'].isnull())& (하프_mg['현금입금'].isnull()), 'T/N'] = '미정산'
...
어드민_GS_pvt = pd.pivot_table(어드민_GS, index='주문_상세', values='총 금액', aggfunc='sum').reset_index()
어드민_GS_pvt
...
GS_mg['귀속월'] = GS_mg['주문_상세'].map(어드민_GS.drop_duplicates('주문_상세').set_index('주문_상세')['귀속월'])
GS_mg.loc[GS_mg['판매가격'].notnull(), '차이']= GS_mg['총 금액'] - GS_mg['판매가격']
GS_mg.loc[GS_mg['현금입금'].notnull(), '차이']= GS_mg['총 금액'] - GS_mg['현금입금']

# +
# 현재 상태 카테고리화 하기
GS_mg.loc[(GS_mg['총 금액'] == 0) & (GS_mg['판매가격'].isnull()), 'T/N'] = 'True'
GS_mg.loc[GS_mg['차이'] == 0, 'T/N'] = 'True'

GS_mg.loc[(GS_mg['총 금액'] < 0) & (GS_mg['판매가격'].isnull()), 'T/N'] = '초기'
GS_mg.loc[(GS_mg['총 금액'] < 0) & (GS_mg['판매가격'] == 0), 'T/N'] = '초기'
GS_mg.loc[(GS_mg['총 금액'] == 0) & (GS_mg['판매가격'] > 0), 'T/N'] = '초기'

GS_mg.loc[(GS_mg['총 금액'] > 0) & (GS_mg['판매가격'].isnull())& (GS_mg['현금입금'].isnull()), 'T/N'] = '미정산'
...
os.chdir('D:\\Python_Files\\매출분석\\온라인\\자사몰_자동화')
...
df = pd.read_excel('GS_조회대상.xlsx')

# 웹 드라이버 설정
driver = webdriver.Chrome(r"D:\\Python_Files\\chromedriver.exe")
driver.implicitly_wait(3)

driver.get('https://xxxxxxxx/login')
time.sleep(1)

driver.find_element_by_css_selector('.mm_form-text').click()
driver.find_element_by_css_selector('#id.textfield').send_keys('xxxxxxxx')
driver.find_element_by_css_selector('#password.textfield').send_keys('xxxxxxxx')
driver.find_element_by_css_selector('.mm_btn.__btn_lg_secondary__.btn_login').click()
time.sleep(1)

result = pd.DataFrame(columns=['자사_주문번호','제휴몰_주문번호', '변경후' ,'비고'])


for i in tqdm(df['자사몰 주문번호']):
    try:
        driver.get(f'https://xxxxxxxx/order/detail/{i}')
        time.sleep(0.5)
        
        g_num = driver.find_element(By.XPATH,'//*[@id="frm_change_status"]/section/div/div/table/tbody/tr/td[2]/ul/li[1]').text
        rec = driver.find_elements(By.TAG_NAME, 'p')
        ext_num = re.findall(r'\d+',rec[-1].text)
        filt_num = [i if (len(i) == 10) & (i != g_num) else '' for i in ext_num][-1]
        
        result = result.append(pd.DataFrame([[i, g_num, filt_num, rec[-1].text]], columns=['자사_주문번호','제휴몰_주문번호', '변경후','비고']), ignore_index=True)
...
os.chdir('D:\\매출분석\\온라인\\자사몰_자동화')
result.to_excel('GS_조회결과.xlsx', index=False)
...
GS_mg.loc[GS_mg['판매가격'].notnull(), '차이']= GS_mg['총 금액'] - GS_mg['판매가격']
GS_mg.loc[GS_mg['현금입금'].notnull(), '차이']= GS_mg['총 금액'] - GS_mg['현금입금']
GS_mg.loc[GS_mg['변경후_금액'].notnull(), '차이'] = GS_mg['총 금액'] - GS_mg['변경후_금액']
...
GS_mg.loc[(GS_mg['총 금액'] > 0) & (GS_mg['판매가격'].isnull()) \
...
# +
writer = pd.ExcelWriter(f'D:\\매출분석\\온라인\\자사몰_제휴사별_분석\\제휴몰_확인_리스트.xlsx', engine='xlsxwriter')
